...
Стандартные схемы сортировки \biblatex плохо работают для многотомных изданий (неверно обрабатывается
поле \bibfield{maintitle}). Поэтому в \biblatexgost определены новые схемы \opt{gostnyt}, 
\opt{gostntvy}. Последняя схема (имя, заголовок, том, год) не имеет аналога в \biblatex
Она по умолчанию включена для
стилей \bibsty{gost-inline} и \bibsty{gost-footnote}. В \bibsty{gost-authoryear} по умолчанию включена 
сортировка \opt{gostnyt} (имя, год, заголовок). В \bibsty{gost-numeric} сортировка всегда в 
порядке цитирования.
...
\item Добавлена обработка длинных списков имён\see{ssec:db:4names}
\item Добавлены схемы сортировки\see{ssec:sorting}
\item Поля volume, book, part, issue теперь могут быть нечисловыми\see{ssec:volsparts}
...
\item Исправлен перевод терминов annotator, withannotator
\item Несколько других исправлений
\end{release}
