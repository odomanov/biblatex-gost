...
Для оформления диссертаций и автореферетов используйте записи типа \bibtype{thesis}. Поле
\bibfield{type} определяет тип диссертации~--- магистерская (\opt{mathesis}), кандидатская
(\opt{phdthesis}), докторская (\opt{docthesis})~--- или тип автореферата: кандидатская
(\opt{phdautoref}), докторская (\opt{docautoref}).
При других значениях поля \bibfield{type}, оно выводится как есть, причём поля
\bibfield{major} и \bibfield{speciality} (см.~ниже) игнорируются.
...
Поля эквивалентны, можно использовать любое из них.
...
title                                           & + & + & + & + & + &   &   &   & + & + & + & + & + & + & + & +\tabularnewline
type                                            &   &   &   &   &   &   &   &   &   & + & + &   & + &   &   & +\tabularnewline
type+major/speciality                           &   &   &   &   &   &   &   &   &   &   &   &   &   &   & + &  \tabularnewline
...
\begin{changelog}
  ...
  \begin{release}{1.12}{??-01-2017}
  \item Слово «наук» теперь не нужно писать в
    \bibfield{major}/\bibfield{speciality}.\see{sec:dissers}
  ...
  \item Определены новые поля \bibfield{speciality}, \bibfield{specialitycode},
    \bibfield{number} для записей типа \bibtype{thesis}.\see{sec:dissers}
