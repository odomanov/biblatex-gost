...
В ГОСТ определён ещё один тип затекстовых ссылок, при котором они связываются с текстом сносками. Этого можно достичь двумя способами. В первом используется стиль
 \bibsty{gost-footnote} при наличии поддержки команды \cmd{endnote} (пакета \sty{endnotes}, стиля документа \sty{memoir} или других). При этом ссылки переносятся в конец текста (или главы, или в любое нужное место). Можно также воспользоваться 
...
\item В \biblatexgost нет понятия параллельного заглавия.
...
Издание может делиться на физически отдельные части, которые могут называться по-разному. \biblatexgost поддерживает деление на тома, книги, части и выпуски (деление только одного уровня; например, тома, разделённые на части, не поддерживаются). Им соответствуют следующие поля базы данных:
