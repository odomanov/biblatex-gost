...
\usepackage{isodate}
\usepackage[bibstyle=gost-standard]{biblatex} % to receive \bbx@gost@version, etc.
%\usepackage{shortvrb}
%\usepackage{pifont}
...
\titlepage{%
  ...
  subtitle={Оформление библиографии согласно ГОСТам\protect\\ 7.0.5--2008 и 7.1--2003},
...
гуманитарных дисциплинах с повышенными требованиями к оформлению библиографии, многие из 
...
\biblatex~2.0\fnurl{http://sourceforge.net/projects/biblatex/files/biblatex-2.0/} 
(который, в свою очередь, требует
\biber~1.0\fnurl{http://sourceforge.net/projects/biblatex-biber/files/biblatex-biber/1.0/}) 
или выше.
...
ссылок, которые выбираются опцией \opt{mergedate} (см.~\secref{sec:opt}).
...
%\item[gost-inline-min, gost-footnote-min, gost-numeric-min, gost-authoryear-min]
...
Разумеется, это не важно для стилей \bibsty{gost-numeric-min} и 
\bibsty{gost-authoryear-min}, которые изменяют оформление библиографии, но не цитат.
...
\centerline{\rule{0pt}{1.5\baselineskip}*~~~*~~~*}
...
\subsection{Термины ГОСТа и поля \biblatexgost}
...
\biblatexgost использует механизм модели данных (опция \opt{datamodel}
при этом не требуется, модель подключается автоматически) и определяет следующие 
...
\fielditem{booktranslator}{name}\vspace{-\itemsep}\vspace{-\parsep}%
\fielditem{bookcommentator}{name}\vspace{-\itemsep}\vspace{-\parsep}%
\fielditem{bookannotator}{name}\vspace{-\itemsep}\vspace{-\parsep}%
\fielditem{bookintroduction}{name}\vspace{-\itemsep}\vspace{-\parsep}%
\fielditem{bookforeword}{name}\vspace{-\itemsep}\vspace{-\parsep}%
\fielditem{bookafterword}{name}\vspace{-\itemsep}\vspace{-\parsep}%
\fielditem{bookoriglanguage}{key}\vspace{-\itemsep}\vspace{-\parsep}%
\listitem{bookcredits}{literal}

Аналогично \bibfield{bookauthor} и \bibfield{booktitle} эти поля 
содержать переводчика, автора комментариев и т.\,д. книги, в состав которой входит 
данная публикация. Поля определены для записей типа 
\bibtype{inbook}, \bibtype{incollection}, \bibtype{inproceedings} 
...
Эти поля могут использоваться, например, при описании сборников переводов классических текстов,
...
то поле выводится только в описании объемлющего документа. В случае переводчика проверяется 
...
используйте поле \bibfield{credits} (см. ниже). 
...
\fielditem{material}{literal}

Общее обозначение материала (см.~\gostbibref{5.2.3}). В \tabref{tab:material} приведены
...
\toprule
\multicolumn{1}{@{}H}{Значение поля \bibfield{material}} &
...
Язык вывода поля  
определяется командой \cmd{gostmateriallanguage}. По умолчанию он совпадает с 
...
\begin{ltxcode}
\renewcommand*{\gostmateriallanguage}{}
\end{ltxcode}
...
\begin{ltxexample}[linewidth=.7\textwidth,escapechar=|]
@BOOK{book,
  ...
...
\optitem[false]{inbookibid}{\opt{true}, \opt{false}}
...
тот же документ, во второй ссылке вместо повторяющихся сведений о документе ставят слова
...
для корректной работы этой опции требуется аккуратное заполнение поля \bibfield{crossref}. 
...
Но если вы изменяете опцию \opt{mincrossrefs}, убедитесь, что записи, 
...
\optitem[gostletter]{mergedate}{\opt{goststrict}, \opt{gostletter}, \opt{gostlabel}}
...
уникальностью ссылок. 
Например, если вы делаете ссылки на тома многотомного издания, то, возможно,  
...
В то же время, пакету \biblatexgost требуются дополнительные поля для хранения количества 
...
переопределение происходит автоматически (благодаря новым функциям \biblatex~2.0/\biber~1.0).}:
...
\begin{release}{0.6}{1-07-2012}
\item Добавлены <<минимальные>> стили\see{sec:styles}
\end{release}

\begin{release}{0.5}{1-07-2012}
\item Добавлено поле \bibfield{material} (общее обозначение материала)\see{sec:newfields} 
\item Восстановлены стандартные значения опции \opt{mergedate}\see{sec:opt}
\item Добавлена опция \kvopt{mergedate}{gostlabel}\see{sec:opt}
\item Добавлена обработка опции \opt{singletitle}\see{sec:opt}
\item Добавлено сокращение <<Там же>>/<<Ibid.>> для  последовательных ссылок на публикации,
  включённые в один и тот же документ. Регулируется опцией \opt{inbookibid}\see{sec:opt}
