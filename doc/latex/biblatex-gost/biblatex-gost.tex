...
%\svnRegisterRevision{22}{v.1.0}% to be used with \svnFullRevision*{\svnrev}
\titlepage{%
...
к заглавию (поле \bibfield{titleaddon}, см.\tabref{tab:gost-biblatex}). Кроме того, не выводятся поля \bibfield{doi}, 
...
следованию ГОСТу (хотя при этом существует опасностью появления неопределённых ссылок).
Стиль устанавливает опции \kvopt{autocite}{inline}, ... 
...
лишь в редких случаях отличается от стандартного, как оно описано в документации
\biblatex.
...
@BOOK{book,
  ...
  shorttitle = {Introduction~\textellipsis\isdot}
}
\end{lstlisting}
