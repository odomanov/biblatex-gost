...
\begin{trivlist}
\item Переход на \biblatex~2.3 и \biber~1.3
\item \bibtype{phdthesis} теперь соответствует кандидатской диссертации,
...
\item Изменён перевод некоторых строк в файле локализации: <<вступительная статья>> вместо
  <<введение>>, почти все строки, связанные с полем \bibfield{related}, а также строки,
...
\item Внесены небольшие изменения в вывод записей типа \bibtype{booklet}, \bibtype{misc},
  \bibtype{online}, \bibtype{periodical}, \bibtype{thesis}, \bibtype{unpublished}
\end{trivlist}
...
в MiKTeX~--- \texttt{initexmf -{}-update-fndb}).

Для работы \biblatexgost версии 0.7 требуется
\biblatex~2.3\fnurl{http://sourceforge.net/projects/biblatex/files/biblatex-2.3/}
и \biber~1.3\fnurl{http://sourceforge.net/projects/biblatex-biber/files/biblatex-biber/1.3/}
...
\bigskip
Обратите внимание, что механизм сортировки \biblatex\slash\biber
...
\optitem{citeurl,biburl}{\opt{true}, \opt{false}}\noitemspace%
\optitem{citeisbn,bibisbn}{\opt{true}, \opt{false}}\noitemspace%
\optitem{citedoi,bibdoi}{\opt{true}, \opt{false}}\noitemspace%
\optitem[\textrm{см.\,\tabref{tab:bibcite}}]{citeeprint,bibeprint}{\opt{true}, \opt{false}}%\noitemspace%
...
\optitem[bib]{related}{\opt{true}, \opt{false}, \opt{bib}, \opt{cite}}
...
т.\,е. поле выводится в библиографии, но не в цитатах.
...
В \biblatexgost диссертации описываются записями трёх видов:
\bibtype{mastersthesis} (магистерская), \bibtype{phdthesis} (кандидатская),
\bibtype{doctorsthesis} (докторская).
Вместо них можно использовать запись \bibtype{thesis} с полем
\bibfield{type}, установленным в \opt{mathesis}, \opt{phdthesis} или \opt{docthesis}
соответственно.
...
\begin{release}{0.7}{20-11-2012}
\item Переход на \biblatex~2.3 и \biber~1.3\see{sec:install}
...
\item Добавлен тип записи \bibtype{docthesis} для докторских диссертаций\see{sec:dissers}
...
\item Улучшена конкатенация строк в сведениях об ответствености при совпадении имён
\item Добавлен файл примеров \texttt{biblatex-gost-examples.pdf}
\item Добавлен раздел документации об проблеме сортировки кириллических\\ записей\see{sec:issues}
