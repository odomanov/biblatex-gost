...
\biblatex позволяет, с некоторыми ограничениями, \hyperref[bibermap]{определять} 
новые поля библиографической
базы данных. В \biblatexgost определены следующие поля. 
...
\subsection{Соответствие терминов ГОСТа и \biblatexgost{а}}
\label{sec:gost-biblatex}

Соответствие терминов ГОСТа и \biblatexgost{а} представлено в следующей таблице.
...
cведения, относящиеся к заглавию}& titleaddon, maintitleaddon,
                                            booktitleaddon \\
%\textit{Параллельное заглавие} & titleaddon, maintitleaddon,
%                                            booktitleaddon \\
...
Сведения об ответственности      & {\spotcolor credits}, editor, editora, editorb, editorc, 
                                    ...
                                    translator \\
\cmidrule(r){1-1}\cmidrule{2-2}
\multicolumn{1}{@{}H}{Область специфических сведений} & specdata \\
%\cmidrule(r){1-1}
\cmidrule(r){1-1}\cmidrule{2-2}
...
%\cmidrule(r){1-1}
Стандартный номер (или его альтернатива) & isbn, isan, ismn, isrn, iswc, doi, eprint, 
...
специальные поля в базе данных или нет прямого соответствия между ГОСТом и \biblatexgost{ом}.
...
\item[Серия.] \label{series}ГОСТ различает (\gostref{6.3.4.2}) сериальные документы 
...
в качестве заголовка соответствующей библиографической 
...
\biblatexgost отслеживает количество имён в списке автоматически, 
однако особенности реализации (см.~\apxref{impl:4names}) не позволяют динамически
изменять опцию \opt{maxnames} (что, впрочем, будет означать отступление от ГОСТа).
...
В частности, \biblatexgost переопределяет следующие поля\footnote{В предыдущих версиях \biblatexgost
для настройки этой функции требовались манипуляции с файлом \file{biber.conf}. Начиная с
\biblatex~2.0~--- соответственно, с \biblatexgost~0.4~--- переопределение происходит автоматически.}:
...
(см.~\secref{sec:db:4names}).
Однако при этом механизм сортировки \biblatex не позволяет обнаружить, что в заголовке
теперь используются не имена, а заглавие. 
Поэтому в \biblatexgost для правильной работы сортировки для публикаций,
...
В \biblatexgost это делается автоматически при помощи функции \cmd{DeclareSourcemap}%
