...
%\usepackage{droid}   % not bad, sc
\usepackage{pscyr}
%\renewcommand{\rmdefault}{fha}% Балтика sc
%\renewcommand{\sfdefault}{far}% arial sc
...
\svnRegisterRevision{128}{Версия 0.4}
\makeatletter
...
\biblatex~2.0\fnurl{https://sourceforge.net/projects/biblatex/files/development/} (который,
в свою очередь, требует \biber~1.0\fnurl{https://sourceforge.net/projects/biblatex-biber/files/biblatex-biber/development/}) или выше.
...
Стиль чувствителен к опции \opt{singletitle} и может не выводить год в ссылке, если имеется
лишь одна публикация данного автора. 
...
\subsection{Соответствие между терминами ГОСТ и \biblatexgost}
...
типа \bibsty{authoryear}. Её цель~--- облегчить поиск публикации в библиографическом списке. 

%\begin{notimplc}
Стандартные стили \biblatex выводят дату в скобках сразу после автора:
...
Данная опция определяет, каким образом выводится дата в самой записи:  
как обычно (\opt{false}),  
выводит дату только для меток с добавленной буквой (\opt{minimum}),  
выводит дату только для меток с добавленной буквой, и если 
она является просто годом (\opt{basic}),  
дата вообще не выводится на своём обычном месте (\opt{maximum}),
то же, но поле \bibfield{issue} всё же выводится (\opt{compact/true}),  
(подробнее см. примеры в документации \biblatex).  
\biblatexgost сохраняет все эти опции и добавляет ещё три. Все они подавляют
вывод года после автора.
...
Для предотвращения неопределённых ссылок (при совпадении автора и года) к году при 
...
\begin{bibexample}
\item \emph{Пушкин А.С.} Сказка о золотом петушке. \ldots --- М., 1963b. --- \ldots
\end{bibexample}

\item[gostlabel]

Ссылки оформляются так же, как и при \kvopt{mergedate}{gostletter} (к году при 
необходимости добавляется латинская буква), но в библиографии записи начинаются с метки, 
применённой при цитировании (автор или заглавие и год в квадратных скобках), а к
году на обычном месте буква не прибавляется:

\begin{bibexample}
\item {[Пушкин, 1990c]} \textit{Пушкин А. С.} Руслан и Людмила. \ldots 1990.~--- \ldots
\end{bibexample} 

\item[goststrict]

Никаких знаков к году не добавляется, что может приводить к 
неопределённым ссылкам, но зато соответствует требованиям ГОСТа. Вам нужно самим следить за
уникальностью ссылок, выбирая соответствующие значения параметра \prm{postnote}. 
Например, если вы делаете ссылку на \bibtype{mvbook}, то в \prm{postnote}, возможно, 
понадобится указать том, часть и т.\,д. или воспользоваться командами типа \cmd{volcite}.
%При установленной опции \kvopt{singletitle}{true} год в ссылке не выводится при наличии 
%единственного произведения данного автора.
%При отсутствии автора
%(когда он в ссылке заменяется названием)
%год выводится в любом случае.
