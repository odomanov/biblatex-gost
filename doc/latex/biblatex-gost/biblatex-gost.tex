...
опция \opt{mergedate}.
По сравнению с \biblatex, она имеет три дополнительных значения \opt{gostletter},
\opt{gostlabel} и 
\opt{goststrict} (см.~\secref{sec:opt}). Последнее соответствует строгому 
...
\begin{bibexample}
\item Doe, John (2000). \textit{Book 1}. \ldots 
\item Doe, John (2003a). \textit{Book 2}. \ldots
\end{bibexample}

Данная опция определяет, каким образом выводится дата в самой записи на её обычном месте:  
как обычно (\kvopt{mergedate}{false}),  
выводит дату только для меток с добавленной буквой (\kvopt{mergedate}{minimum}),  
выводит дату только для меток с добавленной буквой и если 
она является просто годом (\kvopt{mergedate}{basic}),  
дата вообще не выводится на своём обычном месте (\kvopt{mergedate}{maximum}),
то же, но поле \bibfield{issue} всё же выводится (\kvopt{mergedate}{compact/true}),  
(подробнее см. примеры в документации \biblatex).  
\biblatexgost сохраняет все эти опции и добавляет ещё три.

\end{notimplc}

Стиль \bibsty{gost-authoryear}, по сравнению со стандартным \bibsty{authoryear}, имеет три дополнительных значения опции \opt{mergedate}:
...
понадобится указать том, часть и т.\,д.
Данная опция подавляет также вывод года в ссылке при наличии единственного автора или, при 
отсутствии автора, названия публикации.

\item[gostlabel]

В библиографии записи начинаются с метки, аналогичной той, что применяется при цитировании, 
то есть автор (заглавие, если его нет) и год в квадратных скобках\notimpl:

\begin{bibexample}
\item {[Пушкин, 1990]} \textit{Пушкин А. С.} Руслан и Людмила. \ldots 1990.~--- \ldots
\end{bibexample} 
