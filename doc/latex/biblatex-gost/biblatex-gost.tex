...
\item Изменено отношение терминов ГОСТ и полей \biblatex\see{sec:gost-biblatex}
\item Добавлено автоматическое переопределение полей (теперь не нужна настройка \file{biber.conf})\see{bibermap}
\item Добавлена автоматическая обработка длинных списков имён\see{sec:db:4names} 
