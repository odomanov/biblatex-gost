...
Для работы \biblatexgost версии 0.6 и выше требуется 
\biblatex~2.1\fnurl{http://sourceforge.net/projects/biblatex/files/development/} 
...
описанными в \secref{sec:opt}). По умолчанию в минимальных стилях этот вывод отключён.
...
В \biblatexgost опредены следующие 
поля библиографической базы данных, отсутствующие в стандартном \biblatex 
\footnote{Используется дополнительный файл модели данных \file{biblatex-gost.dbx}. 
При этом устанавливать опцию \opt{datamodel}  не требуется, модель подключается автоматически}.   
...
поле выводится только в описании объемлющего документа. В случае переводчика проверяется 
...
\optitem[true]{citeurl,biburl}{\opt{true}, \opt{false}}\vspace{-\itemsep}\vspace{-\parsep}%
\optitem[true]{citeisbn,bibisbn}{\opt{true}, \opt{false}}\vspace{-\itemsep}\vspace{-\parsep}%
\optitem[true]{citedoi,bibdoi}{\opt{true}, \opt{false}}\vspace{-\itemsep}\vspace{-\parsep}%
\optitem[true]{citeeprint,bibeprint}{\opt{true}, \opt{false}}%\vspace{-\itemsep}\vspace{-\parsep}%
...
стилей (см. ниже). По умолчанию все стили, для которых опции 
...
полными стилями и в \opt{false}~--- минимальными. Все эти значения суммированы 
...
\bottomrule
\end{tabularx}
\caption{Значения по умолчанию опций \opt{cite\ldots} и \opt{bib\ldots}}
\label{tab:bibcite}
\end{table}
...
\item Добавлено поле \bibfield{media} (общее обозначение материала)\see{sec:newfields} 
