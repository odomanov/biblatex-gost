...
\documentclass[11pt,a4paper,headings=small,numbers=enddot,english,russian]{ltxdockit}[2011/03/25]
...
\newcommand*{\gostciteref}[1]{\gostcitename, п.\,#1}
...
%\textit{Параллельное заглавие} &  {\spotcolor paratitle} \\
...
\tablesetup
\centering
\begin{tabularx}{.6\textwidth}{@{}V{.3\textwidth}@{}L{.3\textwidth}@{}}
\toprule
...
используется текущий язык библиографической записи или ссылки. 
...
Она переключает экпериментальное поведение, отключенное по умолчанию. Согласно  
\gostciteref{8.11}, в случае последовательных ссылок на публикации, включённые в один и 
...
такое поведение может оказаться нежелательно. Данная опция позволяет его отключить.
...
\begin{release}{0.5}{??-??-2012}
\item Добавлено поле \bibfield{material} для общего обозначения 
  материала\see{sec:newfields} 
\item Исправлена ошибка заглавных букв для нечисловых значений 
  \bibfield{edition}, \bibfield{volume}, \bibfield{book} и т.\,д. 
...
\item Исправлены отклонения от ГОСТ в драйверах \bibtype{booklet}, \bibtype{manual}, 
...
\item Исправлена ошибка, приводящая к неопределённости для публикаций без авторов в
