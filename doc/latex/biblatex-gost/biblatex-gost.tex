...
version\fnurl{http://www.ctan.org/tex-archive/macros/latex/base/lppl.txt}.
This package is
%author"=
maintained.
...
\section{Предназначение пакета}
\label{sec:scope}

Оформление библиографии регулируется тремя основными ГОСТами:

\newlength{\templ}\settowidth{\templ}{\gostcitename~}
\begin{itemize}[labelwidth=\templ,leftmargin=!,align=left]

\item[\gostcitename] Библиографическая ссылка. Общие требования и правила составления.
...
\end{itemize}
Используются также стандарты на сокращения, оформление электронных ресурсов и т.\,д.

Данный пакет предназначен для оформления библиографических \emph{ссылок} по \gostcitename.
Последний использует правила оформления элементов библиографического описания из
  \gostbibname, однако в целом области применения этих двух ГОСТов различаются.
В частности, \gostbibname утверждает о себе:
...
С другой стороны, \gostcitename говорит следующее:
...
\begin{quotation}
  ...
  4.5.1 Полную ссылку, содержащую совокупность библиографических сведений о документе,
    предназначенную для общей характеристики, идентификации и поиска документа --- объекта ссылки,
    составляют по ГОСТ 7.1, ГОСТ 7.82, ГОСТ 7.80.

  4.5.2 Краткую ссылку, предназначенную только для поиска документа --- объекта ссылки,
    составляют на основе принципа лаконизма в соответствии с требованиями настоящего стандарта.
\end{quotation}

Данный пакет предназначен для оформление \emph{краткой} ссылки в соответствии с \gostcitename.
Согласно стандарту, ссылки могут быть внутри текста, в сноске на той же странице или
  за текстом (в конце документа или его части).
В последнем случае список ссылок в конце текста \emph{не является библиографическим указателем}.
ГОСТ говорит об этом вполне ясно:

\begin{quotation}
Совокупность затекстовых библиографических ссыпок не является библиографическим списком или
...
Данный пакет \emph{не предназначен} для оформления библиографического указателя.

\gostcitename отличается от \gostbibname следующим:

\begin{itemize}
  \item Лаконичностью, как уже было сказано.
...
Кроем того, \gostcitename отличается от \gostheadname (оформление заголовков) следующим:
\begin{itemize}
  ...
  \item Заголовок записи в ссылке может содержать имена одного, двух или трех авторов документа.
    ...
    (по \gostheadref{}{5.2} в заголовке используется только одно имя, а по \gostbibref{}{5.2.6.8}
    в сведениях об ответственности обязательно приводят все имена).
\end{itemize}
При этом, если в \gostheadname инициалы в заголовке отделяются от фамилии запятой с пробелом, то в
  \gostcitename~только пробелом, причём специально об этом различии ничего не говорится.
