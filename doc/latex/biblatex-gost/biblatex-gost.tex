...
Permission is granted to copy, distribute and\slash or modify this software under the terms of the \lppl, version 1.3.\fnurl{http://www.ctan.org/tex-archive/macros/latex/base/lppl.txt}
This package is
%author"=
maintained.
...
Для работы \biblatexgost версии 0.7.1 требуются, как минимум,
...
следованию ГОСТу, хотя при этом могут появляться неопределённые ссылок.
...
Заголовок, согласно ГОСТу 7.80---2000 <<Библиографическая запись. Заголовок>>,
состоит из основной части и идентифицирующих признаков.
...
В \biblatex нет понятия заголовка, однако вывод полей \bibfield{author}, \bibfield{editor} и
\bibfield{translator} аналогичен выводу заголовка в ГОСТ. Другими словами можно сказать,
...
\footnote{Программно поле \bibfield{heading} реализовано как ещё одно, фиктивное имя
(т.\,е. имеет тип данных \texttt{name list}). При чтении базы данных оно заключается
в фигурные скобки, поэтому с точки зрения \biblatex представляет собой имя,
состоящее из одной фамилии. В остальном оно обрабатывается так же как поля
\bibfield{author}, \bibfield{editor} и \bibfield{translator}, в частности,
участвует в определении \texttt{labelname} (имея преимущество перед остальными именами:
при наличии поля \bibfield{heading} значение \texttt{labelname} равно ему).}

Примеры использования заголовков см. в \secref{sec:patent}, \secref{sec:standards}.
...
В записях типа \bibtype{online} URL выводится всегда, независимо от значения опций
...
\opt{usetranslator}~--- в заголовке выводятся тем же форматом. Его можно изменить,
...
\endfoot
\endlastfoot
author/translator+others & + & + & + & + &  &  &  &  &  &  &  &  &  &  & \tabularnewline
